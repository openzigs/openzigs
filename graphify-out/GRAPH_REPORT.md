...
## Summary
- 6794 nodes · 10110 edges · 151 communities detected
...
Cohesion: 0.0
Nodes (127): createAirtableReadTools(), createAirtableWriteTools(), createAppWithHandler(), createSeoRouter(), createApp(), getFirecrawlClient(), isBlockedUrl(), createMcpServer() (+119 more)

### Community 2 - "Community 2"
Cohesion: 0.0
Nodes (58): ChannelManager, DiscordChannel, splitDiscordMessage(), MockClient, convertMarkdown(), WebChannel, convertProse(), convertProseText() (+50 more)
...
Cohesion: 0.0
Nodes (56): BaseHTTPRequestHandler, cleanup_old_jobs(), generate(), _idle_timer(), LipSyncRequest, _load_pipeline(), _post_job_cleanup(), _post_to_callback() (+48 more)

### Community 18 - "Community 18"
Cohesion: 0.0
Nodes (32): collectInputs(), createDefaultIO(), parseCsv(), readEnvFile(), readToolsConfig(), runSetup(), writeConfig(), writeEnvFile() (+24 more)
...
- **Thin community `Community 58`** (21 nodes): `WebChatChannel`, `.broadcastApprovalRequest()`, `.clearSessionHistory()`, `.connect()`, `.constructor()`, `.disconnect()`, `.getSocketByChatId()`, `.handleConnection()`, `.isConnected()`, `.onApprovalResponse()`, `.onClear()`, `.onMessage()`, `.restoreSession()`, `.sendApprovalRequest()`, `.sendError()`, `.sendMessage()`, `.sendSessionHistory()`, `.sendStreamChunk()`, `.sendStreamEnd()`, `.sendToolProgress()`, `.sendUserInputRequest()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 59`** (20 nodes): `AirtableClient`, `.buildRecordQueryString()`, `.constructor()`, `.createRecords()`, `.deleteRecords()`, `.listBases()`, `.listRecords()`, `.listTables()`, `.request()`, `.updateRecords()`, `.validateBaseId()`, `RateLimiter`, `.acquire()`, `.constructor()`, `.getQueueSize()`, `.processQueue()`, `mockResponse()`, `airtable-client.test.ts`, `airtable-client.ts`, `types.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 89`** (10 nodes): `CharacterRepository`, `.constructor()`, `.create()`, `.delete()`, `.getAll()`, `.getById()`, `.getByStatus()`, `.migrate()`, `.toModel()`, `.update()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 90`** (10 nodes): `PinterestDigestService`, `.constructor()`, `.extractSummaryMetrics()`, `.formatNumber()`, `.send()`, `.sendDigest()`, `mockChannelManager()`, `mockLogger()`, `pinterest-digest-service.test.ts`, `pinterest-digest-service.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 103`** (9 nodes): `TimelineEditorPage`, `.constructor()`, `.redo()`, `.splitAtPlayhead()`, `.toggleSnap()`, `.undo()`, `.zoomIn()`, `.zoomOut()`, `timeline-editor.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 106`** (8 nodes): `TrimWorker`, `.constructor()`, `.executeFfmpeg()`, `.getJob()`, `.listJobs()`, `.processNext()`, `.submit()`, `.waitForCompletion()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 109`** (8 nodes): `SeoDashboardPage`, `.constructor()`, `.goto()`, `.healthScoreRing()`, `.selectMode()`, `.switchToInnerTab()`, `.switchToTab()`, `seo-dashboard.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 111`** (7 nodes): `FollowUpScheduler`, `.constructor()`, `.processPending()`, `.scheduleForRule()`, `.setSendDm()`, `.start()`, `.stop()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 112`** (7 nodes): `PriceSnapshotRepository`, `.constructor()`, `.ensureTables()`, `.getDb()`, `.getLatestSnapshots()`, `.listMonitoredUrls()`, `.saveSnapshot()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 115`** (7 nodes): `formatCount()`, `formatDate()`, `handleSort()`, `KpiCard()`, `refetchAll()`, `page.tsx`, `page.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 120`** (7 nodes): `BrandTemplatePage`, `.addTemplate()`, `.constructor()`, `.getTemplateCard()`, `.selectTemplate()`, `.setCustomTitle()`, `brand-template.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 121`** (7 nodes): `BRollPanelPage`, `.analyze()`, `.constructor()`, `.getSuggestionCard()`, `.selectDensity()`, `.toggleSuggestion()`, `broll-panel.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 122`** (7 nodes): `PitchWizardPage`, `.constructor()`, `.fillScript()`, `.goto()`, `.pickBrandKit()`, `.pickImageStyle()`, `pitch-wizard.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 123`** (6 nodes): `client()`, `Smoke tests for sadtalker sidecar endpoints.  Issue #919 (Epic #883 cleanup) — c`, `test_gpu_info_returns_503_when_cuda_unavailable()`, `test_gpu_info_returns_503_when_torch_missing()`, `test_gpu_info_returns_payload_when_cuda_available()`, `test_endpoints.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 124`** (6 nodes): `findTransitionBetween()`, `getMusicVolume()`, `isNarrationActiveAtFrame()`, `partitionTimeline()`, `renderSegment()`, `template-composition.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 127`** (6 nodes): `MockMediaRecorder`, `.pause()`, `.resume()`, `.start()`, `.stop()`, `screen-recorder.test.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 128`** (6 nodes): `MockResizeObserver`, `.constructor()`, `.disconnect()`, `.observe()`, `.unobserve()`, `timeline-editor.test.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 162`** (4 nodes): `InlineModelPicker()`, `ModelPickerSelect()`, `useModelsQuery()`, `model-picker-select.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 166`** (4 nodes): `generateRecapPdf()`, `handleDownloadPdf()`, `pdf-generator.ts`, `recap-screen.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 174`** (4 nodes): `PresenterDetailPage`, `.constructor()`, `.gotoWithMocks()`, `presenter-detail.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 175`** (4 nodes): `createInitialPresenterState()`, `presenterReducer()`, `usePresenterState()`, `use-presenter-state.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 204`** (3 nodes): `addEvent()`, `panelReducer()`, `subagent-live-panel.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 208`** (3 nodes): `BrandKitEditorDialog`, `.constructor()`, `brand-kit-editor.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 209`** (3 nodes): `BrandKitPicker`, `.constructor()`, `brand-kit-picker.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
