...
## Summary
- 7047 nodes · 10445 edges · 152 communities detected
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 761 edges (avg confidence: 0.78)
...
Cohesion: 0.0
Nodes (113): platform(), createAudioRouter(), extractErrorMessage(), formatSidecarErrorMessage(), parseJsonIfPossible(), buildApp(), createTestDb(), createCharacterRouter() (+105 more)

### Community 1 - "Community 1"
Cohesion: 0.0
Nodes (129): createAirtableReadTools(), createAirtableWriteTools(), getFirecrawlClient(), isBlockedUrl(), ChannelManager, createMcpServer(), registerMcpTools(), isRiskLevel() (+121 more)
...
Cohesion: 0.0
Nodes (54): createOutboxRouter(), createAppWithCopilot(), VideoGenService, clampToFluxQRecommendedDims(), defaultSidecarUrl(), refreshFluxQGpuAvailable(), refreshFluxQRecommendedDims(), deriveFallbackBackgroundPrompt() (+46 more)
...
Cohesion: 0.0
Nodes (57): BaseHTTPRequestHandler, cleanup_old_jobs(), generate(), gpu_info_endpoint(), _idle_timer(), LipSyncRequest, _load_pipeline(), _post_job_cleanup() (+49 more)
...
Cohesion: 0.0
Nodes (26): createDirectorRouter(), buildApp(), createMockCopilot(), buildApp(), createMockCopilot(), createMockToolRegistry(), formatElapsed(), createVideoTools() (+18 more)
...
- **Thin community `Community 63`** (17 nodes): `PinterestTrackerRepository`, `.addContentIdea()`, `.addSnapshot()`, `.constructor()`, `.deleteContentIdea()`, `.deleteTrackedPin()`, `.getLatestSnapshot()`, `.getPinPerformanceSummary()`, `.getSnapshots()`, `.getTrackedPin()`, `.listContentIdeas()`, `.listTrackedPins()`, `.migrate()`, `.trackPin()`, `.updateIdeaStatus()`, `.updateLastChecked()`, `.updatePinStatus()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 64`** (17 nodes): `asString()`, `formatApprovalContext()`, `truncate()`, `ApprovalQueue`, `.cleanupApproval()`, `.constructor()`, `.expireApproval()`, `.finalizeDecision()`, `.get()`, `.handleDecision()`, `.list()`, `.requestApproval()`, `.scheduleCleanup()`, `approval-formatters.test.ts`, `approval-formatters.ts`, `approval-queue.test.ts`, `approval-queue.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 84`** (11 nodes): `WorkflowsPage`, `.constructor()`, `.getCanvasEdges()`, `.getCanvasNodes()`, `.getFitViewButton()`, `.getNodeByText()`, `.getWorkflowNameInput()`, `.getZoomInButton()`, `.getZoomOutButton()`, `.goto()`, `workflows.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 86`** (10 nodes): `CharacterRepository`, `.constructor()`, `.create()`, `.delete()`, `.getAll()`, `.getById()`, `.getByStatus()`, `.migrate()`, `.toModel()`, `.update()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 87`** (10 nodes): `PinterestDigestService`, `.constructor()`, `.extractSummaryMetrics()`, `.formatNumber()`, `.send()`, `.sendDigest()`, `mockChannelManager()`, `mockLogger()`, `pinterest-digest-service.test.ts`, `pinterest-digest-service.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 88`** (10 nodes): `InpaintingPage`, `.constructor()`, `.goto()`, `.selectCharacter()`, `.selectCharacterByLabel()`, `.selectModel()`, `.setPrompt()`, `.submit()`, `.uploadSampleImage()`, `inpainting.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 89`** (10 nodes): `IntegrationsPage`, `.constructor()`, `.expandSection()`, `.goto()`, `.mockSaveSuccess()`, `.mockStatusConfigured()`, `.mockStatusNotConfigured()`, `.mockTestFailure()`, `.mockTestSuccess()`, `integrations.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 90`** (10 nodes): `WorkbenchPage`, `.closeSeoDialog()`, `.constructor()`, `.fillTargetKeyword()`, `.fillTargetUrl()`, `.goto()`, `.openSeoDialog()`, `.selectSearchProvider()`, `.submitAnalysis()`, `workbench.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 96`** (9 nodes): `HandoffManager`, `.closeHandoff()`, `.constructor()`, `.escalate()`, `.forwardToThread()`, `.getContactByThread()`, `.handleAdminReply()`, `.rebuildThreadMap()`, `.registerChannel()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 109`** (7 nodes): `run-e2e.ps1`, `Test-PortInUse()`, `Wait-ForPort()`, `Write-Err()`, `Write-Info()`, `Write-Ok()`, `Write-Warn()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 110`** (7 nodes): `FollowUpScheduler`, `.constructor()`, `.processPending()`, `.scheduleForRule()`, `.setSendDm()`, `.start()`, `.stop()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
