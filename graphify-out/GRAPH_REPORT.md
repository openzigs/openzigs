...
## Summary
- 7173 nodes · 10654 edges · 155 communities detected
...
Cohesion: 0.0
Nodes (106): platform(), createCharacterRouter(), getCharactersDir(), getImageGenSidecarUrl(), getImageGenToken(), getPhotosDir(), pollTrainingStatus(), resumeStaleTrainingPolls() (+98 more)

### Community 1 - "Community 1"
Cohesion: 0.0
Nodes (123): createAirtableReadTools(), createAirtableWriteTools(), createTemplatesRouter(), setup(), getFirecrawlClient(), isBlockedUrl(), createMcpServer(), registerMcpTools() (+115 more)

### Community 2 - "Community 2"
Cohesion: 0.0
Nodes (261): BaseModel, Dataset, AsyncGenerateRequest, AsyncImg2ImgRequest, AsyncKontextRequest, _bg_generate(), _bg_img2img(), _bg_kontext() (+253 more)
...
Cohesion: 0.0
Nodes (54): createOutboxRouter(), createAppWithCopilot(), ChannelManager, WebChannel, FakeIOServer, FakeSocket, isExpired(), isRateLimitError() (+46 more)

### Community 7 - "Community 7"
Cohesion: 0.0
Nodes (57): createRemoteNodesRouter(), defaultConfigPath(), buildApp(), computeSignature(), verifyHmacCallback(), createQueueCallbackRouter(), createQueueRouter(), buildApp() (+49 more)
...
Cohesion: 0.0
Nodes (48): createApp(), FakeCopilot, createApp(), createAdminRouter(), defaultConfigPath(), defaultEnvPath(), exchangeLinkedInCode(), exchangePinterestCode() (+40 more)

### Community 10 - "Community 10"
Cohesion: 0.0
Nodes (84): AuditHistoryRepository, analyzeContent(), analyzeContentFreshness(), computeKeywordDensity(), escapeCsv(), exportDuplicatesCsv(), exportThinContentCsv(), findDuplicateGroups() (+76 more)
...
Cohesion: 0.0
Nodes (61): cleanup_old_jobs(), _detect_host_ram_gb(), generate(), _idle_timer(), LipSyncRequest, _load_pipeline(), _post_job_cleanup(), _post_to_callback() (+53 more)
...
## Knowledge Gaps
- **436 isolated node(s):** `Verify _get_max_frames_for_model returns sane values for both single-GPU and poo`, `Call Pinterest API v5.`, `Call local OpenZigs tracker API.`, `Create a single pin on Pinterest.`, `Add the real pin to the local tracker and take an initial snapshot.` (+431 more)
...
- **Thin community `Community 100`** (9 nodes): `CompetitorRepository`, `.addCompetitor()`, `.constructor()`, `.ensureTables()`, `.getDb()`, `.getLatestSnapshots()`, `.listCompetitors()`, `.removeCompetitor()`, `.saveSnapshot()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 103`** (9 nodes): `TimelineEditorPage`, `.constructor()`, `.redo()`, `.splitAtPlayhead()`, `.toggleSnap()`, `.undo()`, `.zoomIn()`, `.zoomOut()`, `timeline-editor.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 114`** (7 nodes): `FollowUpScheduler`, `.constructor()`, `.processPending()`, `.scheduleForRule()`, `.setSendDm()`, `.start()`, `.stop()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 117`** (7 nodes): `formatCount()`, `formatDate()`, `handleSort()`, `KpiCard()`, `refetchAll()`, `page.tsx`, `page.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 125`** (6 nodes): `client()`, `Smoke tests for sadtalker sidecar endpoints.  Issue #919 (Epic #883 cleanup) — c`, `test_gpu_info_returns_503_when_cuda_unavailable()`, `test_gpu_info_returns_503_when_torch_missing()`, `test_gpu_info_returns_payload_when_cuda_available()`, `test_endpoints.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 126`** (6 nodes): `findTransitionBetween()`, `getMusicVolume()`, `isNarrationActiveAtFrame()`, `partitionTimeline()`, `renderSegment()`, `template-composition.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 129`** (6 nodes): `MockMediaRecorder`, `.pause()`, `.resume()`, `.start()`, `.stop()`, `screen-recorder.test.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 130`** (6 nodes): `MockResizeObserver`, `.constructor()`, `.disconnect()`, `.observe()`, `.unobserve()`, `timeline-editor.test.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 134`** (6 nodes): `AdminPage`, `.constructor()`, `.fetchPlatformApi()`, `.getPlatformBadges()`, `.goto()`, `admin.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 135`** (6 nodes): `AnalyticsDashboardPage`, `.constructor()`, `.goto()`, `.refresh()`, `.selectPeriod()`, `analytics-dashboard.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 136`** (6 nodes): `PitchLibraryPage`, `.constructor()`, `.deckTitle()`, `.errorDetail()`, `.goto()`, `pitch-library.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 145`** (5 nodes): `async()`, `blankForm()`, `contrastRatio()`, `relativeLuminance()`, `brand-kit-editor.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 148`** (5 nodes): `AudioCleanerPage`, `.clean()`, `.constructor()`, `.selectAggressiveness()`, `audio-cleaner.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 149`** (5 nodes): `FramingPanelPage`, `.constructor()`, `.expectPanelVisible()`, `.gotoStudio()`, `framing-panel.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 150`** (5 nodes): `NLEExportPage`, `.constructor()`, `.doExport()`, `.selectFormat()`, `nle-export.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 153`** (4 nodes): `transition-mapper.test.ts`, `transition-mapper.ts`, `getDefaultTransition()`, `mapTransition()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 166`** (4 nodes): `InlineModelPicker()`, `ModelPickerSelect()`, `useModelsQuery()`, `model-picker-select.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 170`** (4 nodes): `generateRecapPdf()`, `handleDownloadPdf()`, `pdf-generator.ts`, `recap-screen.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 178`** (4 nodes): `PresenterDetailPage`, `.constructor()`, `.gotoWithMocks()`, `presenter-detail.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 179`** (4 nodes): `createInitialPresenterState()`, `presenterReducer()`, `usePresenterState()`, `use-presenter-state.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 182`** (4 nodes): `AdminPage()`, `usePlatform()`, `page.tsx`, `use-platform.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 183`** (3 nodes): `log()`, `main()`, `graphify-ast-build.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 185`** (3 nodes): `getPositionStyle()`, `TextOverlayItem()`, `text-overlay-layer.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 191`** (3 nodes): `handleSave()`, `parseList()`, `channels-panel.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 212`** (3 nodes): `makeFakeStream()`, `makeRemotePeer()`, `video-grid.test.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 214`** (3 nodes): `addEvent()`, `panelReducer()`, `subagent-live-panel.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 218`** (3 nodes): `BrandKitEditorDialog`, `.constructor()`, `brand-kit-editor.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 219`** (3 nodes): `BrandKitPicker`, `.constructor()`, `brand-kit-picker.page.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 221`** (3 nodes): `mockEmptySeoApis()`, `mockSeoApis()`, `seo-dashboard.spec.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 225`** (3 nodes): `middleware()`, `middleware.ts`, `verifyGuestToken()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 227`** (2 nodes): `_test_caps.py`, `Verify _get_max_frames_for_model returns sane values for both single-GPU and poo`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 228`** (2 nodes): `test-f5tts-direct.py`, `Direct test of F5-TTS inference to debug tensor mismatch.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.