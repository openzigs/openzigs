...
- **Unified test-coverage reporting (Epic #1117):** Backend (`src/`) and desktop (`desktop/src/`) Vitest now emit `coverage/backend/coverage-final.json`; UI Vitest now emits `ui/coverage/coverage-final.json`. New `scripts/merge-coverage.mjs` combines both into `coverage/coverage-final.json` + `coverage/coverage-summary.json` via `istanbul-lib-coverage`. New `scripts/check-coverage.mjs` enforces per-area line thresholds from `coverage/thresholds.json` (backend >=70%, ui >=22%, desktop >=0%) and fails CI on regression. New root scripts: `pnpm coverage:report`, `pnpm coverage:check`. CI gains a `coverage` job that consumes uploaded artifacts from `api` + `ui`, merges, and gates the build (#1134, #1127, #1167).
- **Cloudflare Access service-token authentication for Remote Media Worker Nodes (#1097):** Per-node `cfAccessClientId` / `cfAccessClientSecret` fields are now sent as `CF-Access-Client-Id` / `CF-Access-Client-Secret` headers on Admin “Test” probes and all QueueMaster job dispatches, alongside the existing Bearer token. Configurable via `/admin` → Remote Media Worker Nodes. See `docs/REMOTE_NODES_SETUP.md` → “Cloudflare Access service tokens”.
...
- **Dependency CVE sweep (Dependabot batch):** Force-bumped vulnerable transitive packages via `pnpm.overrides`: `protobufjs >=8.2.0`, `@protobufjs/utf8 >=1.1.1` (prototype pollution / parsing), `hono >=4.12.18` (auth bypass), `@github/copilot >=1.0.43` (auth/proxy CVEs), `fast-uri >=3.1.2`, `basic-ftp >=5.3.1`, `ip-address >=10.1.1`, `qs >=6.15.2`, plus `mermaid >=11.15.0` and `next ^15.5.18` (cache/SSRF). torch 2.5.1 remains pinned in lipsync MPS due to a documented attention regression in 2.6 under MPS (#1103); the CVE risk is bounded since checkpoints are loaded only from trusted HuggingFace repos pinned by revision.
- **Stack-trace exposure removed (`py/stack-trace-exposure`, CodeQL):** `sidecars/v2a/server_cuda.py` and `sidecars/lipsync/server_cuda.py` `gpu-info` handlers no longer return raw `str(exc)` to clients; they log full exceptions server-side and return a generic error token.
...
- **Local LLM panel: model placeholder now suggests `gemma4:e2b-mlx-bf16`** instead of `gemma4:26b`. The 26B variant requires ~36 GB unified memory at INT4 and is the recommended model only on Linux + NVIDIA. The MLX-native E2B build is a better default hint for Apple Silicon users (the platform most local-LLM users run). Hardware-aware setup-wizard recommendations are unchanged.
