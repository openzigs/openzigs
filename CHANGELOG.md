...
- **Admin: vLLM unavailability parity on Apple Silicon (Closes #1078).** The `/admin` Local LLM Provider combobox now disables and prefixes the vLLM option with a no-entry sign + reason on `darwin/arm64` (matching the wizard's label-don't-hide behaviour) and renders an inline `role="note"` notice below the picker. The Local vLLM (TP=2) panel replaces its picker + Start button with the same notice on unsupported hosts and stops the 5-second status polling so the badge no longer stays permanently red. `GET /api/system/platform` now returns `vllmSupported: boolean` + `vllmUnsupportedReason: string | null` derived from `gpuKind` -- single source of truth shared between the wizard, the provider picker, and the vLLM panel. Dropped a duplicate `SystemRequirementsCard` heading wrapper.
