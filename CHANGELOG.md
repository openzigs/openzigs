...
- **LTX-2 native audio+video sidecar on port 5013 (#939 follow-up to PR #940):** New `sidecars/ltx2/` FastAPI service that wraps Lightricks' upstream `ltx_pipelines.distilled` CLI from the [Lightricks/LTX-2](https://github.com/Lightricks/LTX-2) monorepo. Produces single-pass MP4s with **natively muxed audio** (H.264 video + AAC 48 kHz audio in one shot, no v2a post-processing). Smoke-validated end-to-end on a single RTX 3060 12 GB with `--offload cpu` and ~32 GB system RAM (BF16 layer streaming): 512×512×25 frames at 24 fps in ~35 s wall clock. Provisioning is idempotent via `bash sidecars/ltx2/setup.sh` (clones monorepo → `uv sync` → downloads ~53 GB of model artefacts: distilled FP8 checkpoint, spatial upsampler, gated Gemma 3 12B text encoder). Endpoints: `/health` (env-readiness probe), `/gpu-info` (lightweight `nvidia-smi` summary), `/generate` (202 + job id), `/status/{id}`, `/unload`. Per-job subprocess invocation lets the OS reclaim ~30 GB CPU RAM between jobs (rationale documented in module docstring). Bearer-token auth, SSRF-hardened loopback-only callbacks, and CodeQL-friendly path/argv containment.
- **`served_by_sidecar` registry marker on `ltxv-2-22b-distilled`:** The worker's video model registry now points the LTX-2 entry at `http://localhost:5013` instead of the stale `unavailable=True` block. The worker's in-process `generate_video_ltx2()` refuses requests for models carrying this marker so the orchestrator must route directly to the dedicated sidecar.
- **Native audio capability gating reworked:** `/capabilities` `audio_modes` includes `"native"` based on a live `/health` probe to the ltx2 sidecar (status 200 + `ready: true`), replacing the old hardware-only `pooled_vram_gb >= 24` gate. Native audio now works on 12 GB single-GPU hardware via the upstream CPU-offload path.
- **Smoke harness `scripts/ltx2_smoke.sh`:** Reproducible end-to-end LTX-2 generation test invokable independently of the sidecar HTTP layer; writes `/tmp/ltx2-out/smoke.mp4` for `ffprobe` verification.
...
### Fixed (Issue #939 — Epic #924 integration gaps)

- **Native LTX-2 audio no longer gated on `LTX_ALLOW_AUDIO` (walkthrough fix, verified 2026-04-24):** The worker's `/capabilities` probe was suppressing `"native"` from `audio_modes` when `LTX_ALLOW_AUDIO` was unset, even when the dedicated ltx2 sidecar (port 5013) was healthy. The flag was originally an opt-in for the worker's *in-process* audio path which competes for VRAM; the sidecar is its own opt-in (it has to be launched and pass `/health`). Now the native probe runs unconditionally — if the sidecar replies `ready: true`, native is advertised. Adds regression test `test_audio_modes_includes_native_even_when_ltx_allow_audio_false`. Surfaces in the studio composer's Audio dropdown without requiring the user to set an env var their normal launcher omits.
- **Studio composer Sync-audio gate now keys off live capabilities, not substring matching (walkthrough fix, verified 2026-04-24):** `syncAudioModelSelected` previously gated the Sync option on `form.model_repo.includes("LTX-Video-2") || form.model_repo.includes("ltxv-2-22b")`, but neither substring matched any value the model picker actually emitted (`AITRADER/ltx2-distilled-4bit-mlx`, `dgrauet/ltx-2.3-mlx-distilled-q4`), making Sync permanently disabled. The gate now (a) consults `capabilitiesQuery.data.models[]` and unlocks when any selected model has `synchronized_audio: true`, and (b) honours an `/ltx-?2|ltxv-?2/i` heuristic over both `model_repo` and `imageModel` so future picker entries don't silently regress. Tooltip messaging refers to the sidecar's readiness state instead of the obsolete pooled-VRAM threshold.
- **v2a (MMAudio) sidecar startup + audio-mode auto end-to-end (#940 follow-up, verified 2026-04-23):** Two latent bugs blocked `audio_mode: "auto"` from ever producing a real WAV: (1) `sidecars/v2a/server_cuda.py` `__main__` parsed args but never called `uvicorn.run()`, so the sidecar exited silently on launch with an empty log; (2) for sub-second-precision videos (e.g. ffmpeg's `d=4` libx264 boundary producing 3.96 s), `mmaudio.eval_utils.load_video` returns one fewer frame than `seq_cfg` expects, tripping `assert clip_f.shape[1] == self._clip_seq_len` deep in `mmaudio/model/networks.py`. Fix: call `load_video` first, derive `actual_clip_len` / `actual_sync_len` from the returned tensors, scale `actual_latent_len` proportionally, then call `net.update_seq_lengths(...)`. End-to-end smoke test now produces a 354 KB / 44.1 kHz mono / 4.02 s WAV from a synthetic 5 s video.
- **LTX worker registry corrected for two upstream-broken HF repos (#940 follow-up, verified 2026-04-23):** Two registry entries were pointing at HuggingFace repos that **do not exist** (HTTP 404 even with a valid token). Both are now marked `unavailable: True` with explicit `unavailable_reason`, and `/models`, `/capabilities`, and `/generate` filter / reject them with HTTP 503 instead of letting `from_pretrained()` fail cryptically deep in diffusers. (1) `ltxv-2b-096-distilled` previously pointed at `Lightricks/LTX-Video-0.9.6-distilled` — Lightricks publishes 0.9.6-distilled only as a raw `.safetensors` inside the umbrella `Lightricks/LTX-Video` repo (for ComfyUI), with no diffusers-layout snapshot. Use `ltxv-2b-legacy` or `ltxv-13b-097-distilled` instead. (2) `ltxv-2-22b-distilled` previously pointed at `Lightricks/LTX-Video-2-22B-distilled` — the real public repo is `Lightricks/LTX-2` (19B parameters, NOT 22B; pipeline is `LTX2Pipeline` from diffusers 0.38, NOT `LTXConditionPipeline`). Even with the corrected `hf_id`/`pipeline_class`, `LTX2Pipeline.from_pretrained(...)` fails because `Lightricks/LTX-2/model_index.json` references `from ltx2 import LTX2TextConnectors` and `from ltx2 import LTX2Vocoder`, but Lightricks has not published an `ltx2` PyPI package nor exposed those classes via their `ltx-core` / `ltx-pipelines` packages (import names `ltx_core` / `ltx_pipelines`). Registry key preserved as `ltxv-2-22b-distilled` for backwards compatibility with existing tests / UI / docs.

- **v2a (MMAudio) sidecar now started by `sidecars/start-cuda-sidecars.sh` and `scripts/dev-clean.ps1` (port 5012):** The sidecar code from #925 was never wired into either startup path, so any video job submitted with `audio_mode="auto"` silently fell through. Both scripts now launch the v2a venv on `V2A_GPU_INDEX` (defaults to `cuda:0` on multi-GPU hosts), include 5012 in the kill-port loop, and surface it in the health-check banner. `docker-compose.yml` gains an opt-in `v2a-sidecar` service under the `v2a` profile for Linux hosts with the NVIDIA Container Toolkit.
- **Worker `/capabilities` refactored to a flat schema:** Old shape (`pooling{}`, `per_device[]`, `max_frames{}`, `env{}`) is replaced by the spec from #939 — `gpu_count`, `gpus[]`, `pooled_vram_gb`, `pooling_active`, `pooling_mode`, `transformer_device`, `encoder_device`, `vae_device`, `models[]`, `audio_modes[]`. `audio_modes` now reflects what the runtime can *actually* do: `"auto"` is included only when v2a (5012) responds to `/health`, `"music"` only when the music sidecar (5009) responds, and `"native"` only when LTX-2 weights are present + `LTX_ALLOW_AUDIO=1` + pooled VRAM ≥ 24 GB. No committed consumer relied on the old shape.
- **v2a failures now mark the parent job degraded (`audio_status: "failed"`):** The post-completion v2a hook in `QueueMaster` was fire-and-forget (`void dispatchV2aJob(...).catch(...)`), so connection refusals or 5xx errors were logged and forgotten. The hook is now `await`ed (the dispatch is bounded by the v2a-client's existing 10 s `AbortSignal.timeout`); failures are merged into `result_metadata` via the new `MediaQueueRepository.updateResultMetadata()` and a refreshed `job:complete` event is re-emitted. The gallery shows a yellow "No audio" chip on completed video assets when this flag is set, with the underlying error in the tooltip.
- **Worker GPU visibility via new `WORKER_POOLING_MODE` env (default `auto`):** When set to `auto` and `GPU_COUNT ≥ 2`, the WSL launcher now exports `CUDA_VISIBLE_DEVICES=0,1` to the LTX worker so its `_is_pooling_active()` check can shard transformer/encoder/VAE across both cards. Setting `WORKER_POOLING_MODE=off` falls back to the legacy single-GPU pin via `WORKER_GPU_INDEX`. The worker logs the resolved visibility + pooling state at startup (`[pooling] CUDA_VISIBLE_DEVICES=… LTX_POOLING_MODE=…`).
- **Dead `/generate-extended` endpoint removed:** The orchestrator (`src/queue/queue-master.ts`) handles extended-duration jobs by submitting a sequence of standard `/generate` calls with `init_image` chained from `/last-frame` and stitching with ffmpeg client-side. The worker-side `/generate-extended` endpoint, `run_extended_generation_job`, and helpers (`_extract_last_frame_b64`, `_concat_clips`, `_probe_duration`) duplicated that work and were never invoked. A follow-up issue tracks moving stitching back into the worker for lower IPC overhead if profiling justifies it.
- **`/gpu-info` now exposes pooling state:** Added `pooling_mode`, `pooling_active`, `transformer_device`, `encoder_device`, `vae_device`, `pooled_vram_gb`, and a per-GPU `gpus[]` array (index/name/vram_gb/free_gb) when `device_count > 1`. The admin GPU panel can now render the dual-card view without scraping logs.
- **`flux-dev` LoRA preset resolution lowered to 512:** The 768×768 default OOM'd 12 GB cards even with 4-bit QLoRA. Per the Q2 2026 LoRA inpainting research, 512 is the safe default; users with ≥ 24 GB pooled VRAM can override per-job. Description updated accordingly.
