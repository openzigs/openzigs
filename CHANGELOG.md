...
- **Admin → Models page now signposts the existing remote LTX node configuration when the local sidecar isn't reachable on macOS (Closes #1082).** Instead of a generic "Sidecar returned HTTP 404" 502, the banner now reads "LTX worker not configured" with the URL the proxy actually tried and a CTA link to **Admin → Video Generation Node** (`/admin#video-gen-node`). Triggers only when the platform is macOS AND `videoGen.mode === "local"` — Windows/Linux and already-configured network-mode setups keep the existing generic copy. Backend `/api/admin/capabilities` 502 body now includes `{ mode, url, isMacLocal, hint? }`; status code is unchanged.
