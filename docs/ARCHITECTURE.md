...
- **TypeScript proxies** (`src/sidecars/error-normalizer.ts`): `normalizeSidecarError(rawBody, status?)` walks any response shape (depth-capped at 6, message-capped at 500 chars) and returns `{ userMessage, code?, hint?, raw, status? }`. Tracebacks are reduced to their last line; HTML pages are replaced with a generic message; absolute paths (`/Users/...`, `/home/...`, `C:\Users\...`, bare `C:\...`) and env-var-shaped assignments (`UPPER_SNAKE_CASE=value`) in the user-facing message are redacted while the full body is kept in `raw` for server-side audit logging. `sidecarFetch()` wraps `fetch` so proxies can `throw new SidecarProxyError(...)` and let `src/api/*` convert it to a uniform JSON response. Backed by 17 fixtures in `src/sidecars/__fixtures__/error-fixtures.ts`.
- **Admin restart route** (`POST /api/admin/ai-sidecars/:name/restart`): IP-rate-limited (5 req/min) and returns a generic `{ error: "Failed to restart sidecar. Check server logs.", code: "restart_failed" }` envelope on failure — raw exception detail is logged server-side, never returned to the client.
