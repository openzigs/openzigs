...
describe("DigestGenerator", () => {
  ...
  describe("RAG health in status markdown (#218)", () => {
    it("includes healthy RAG section when ragHealth is provided", () => {
      const generator = new DigestGenerator();
      // Generate record synchronously for markdown rendering
      const record = {
        timestamp: "2026-03-30T12:00:00Z",
        period: { from: "2026-03-30T00:00:00Z", to: "2026-03-30T12:00:00Z" },
        taskSummary: { completed: 5, failed: 0, cancelled: 0, successRate: 1 },
        tokenBurn: null,
        promptAudit: null,
      ...
      expect(md).toContain("## Knowledge Base Health");
      expect(md).toContain("✅ Healthy");
      expect(md).toContain("50 indexed (500 chunks)");
      expect(md).toContain("2 files");
      expect(md).toContain("Running");
    });

    it("shows degraded when RAG has warnings", () => {
      const generator = new DigestGenerator();
      const record = {
        timestamp: "2026-03-30T12:00:00Z",
        period: { from: "2026-03-30T00:00:00Z", to: "2026-03-30T12:00:00Z" },
      ...
      const md = generator.generateStatusMarkdown(record, {
        ...
        lastIndexedAt: null,
        alerts: [{ type: "rag-ingestion-down", priority: "warning", message: "stopped" }],
      });
    ...
    it("shows unreachable when DB is down", () => {
      const generator = new DigestGenerator();
      const record = {
        timestamp: "2026-03-30T12:00:00Z",
        period: { from: "2026-03-30T00:00:00Z", to: "2026-03-30T12:00:00Z" },
      ...
      const md = generator.generateStatusMarkdown(record, {
        ...
        pendingDocuments: 0,
        lastIndexedAt: null,
        alerts: [{ type: "rag-db-unreachable", priority: "critical", message: "DB down" }],
      });

      expect(md).toContain("❌ Unreachable");
    });

    it("does not include RAG section when ragHealth is null", () => {
      const generator = new DigestGenerator();
      const record = {
        timestamp: "2026-03-30T12:00:00Z",
        period: { from: "2026-03-30T00:00:00Z", to: "2026-03-30T12:00:00Z" },
      ...
      const md = generator.generateStatusMarkdown(record);
      expect(md).not.toContain("Knowledge Base Health");
    });
  });
});
