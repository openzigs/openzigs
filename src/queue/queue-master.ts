...
export class QueueMaster extends EventEmitter {
  ...
  /**
   * Multi-segment job tracking.
   * Maps parentJobId → { totalSegments, completedSegments, segmentJobIds, segmentFiles }
   */
  private segmentTracker = new Map<
    string,
    {
      totalSegments: number;
      completedSegments: number;
      segmentJobIds: string[];
      segmentFiles: (string | null)[];
  ...
  /**
   * Decompose a video job with duration > 4s into N 4-second segment sub-jobs.
   * The first segment uses the original prompt/params; subsequent segments
   * are dispatched as chained image-to-video jobs after the previous segment
   * completes (via handleSegmentCompletion).
   *
   * @returns the first segment job ID, or null if decomposition isn't needed
   */
  decomposeMultiSegmentJob(
    ...
    const segmentCount = computeSegmentCount(durationSec);
    if (segmentCount <= 1) return null;

    logger.info(
      `[QueueMaster] Decomposing job ${parentJobId} into ${segmentCount} segments (${durationSec}s total)`,
    );

    const segmentJobIds: string[] = [];
    const segmentFiles: (string | null)[] = new Array(segmentCount).fill(null);

    // Create only the first segment job — subsequent segments are created
    // by handleSegmentCompletion after chaining via /last-frame.
    const firstSegmentPayload: MediaJobPayload = {
      ...payload,
      num_frames: MAX_VIDEO_FRAMES,
      video_duration: SEGMENT_DURATION_SEC,
      segmentIndex: 0,
      totalSegments: segmentCount,
      parentJobId,
      // First segment never has audio — audio is added after stitching
      audio: false,
    };

    const firstSegmentJob = this.repo.createJob({
      type: payload.init_image ? "img2video" : "txt2video",
      payload: firstSegmentPayload,
      model: payload.model_repo ?? undefined,
      projectId: undefined,
      priority: 10, // Higher priority for segment sub-jobs
    });

    segmentJobIds.push(firstSegmentJob.id);

    this.segmentTracker.set(parentJobId, {
      totalSegments: segmentCount,
      completedSegments: 0,
      segmentJobIds,
      segmentFiles,
    ...
    logger.info(
      `[QueueMaster] First segment job created: ${firstSegmentJob.id} (0/${segmentCount})`,
    );
    return firstSegmentJob.id;
  }

  /**
   * Handle completion of a single segment in a multi-segment job.
   * If more segments remain, extract the last frame and chain the next segment.
   * If all segments are done, stitch them together.
   */
  async handleSegmentCompletion(
    _segmentJobId: string,
    segmentJob: MediaJob,
    resultFilePath: string,
  ): Promise<void> {
    const parentJobId = segmentJob.payload.parentJobId;
    if (!parentJobId) return;

    const tracker = this.segmentTracker.get(parentJobId);
    if (!tracker) {
      logger.warn(
        `[QueueMaster] Segment completion for unknown parent: ${parentJobId}`,
      );
      return;
    }

    const segmentIndex = segmentJob.payload.segmentIndex ?? 0;
    tracker.segmentFiles[segmentIndex] = resultFilePath;
    tracker.completedSegments++;

    logger.info(
      `[QueueMaster] Segment ${segmentIndex + 1}/${tracker.totalSegments} complete for parent ${parentJobId}`,
    );

    // Report aggregate progress
    this.reportProgress(parentJobId, {
      stage: "segment",
      progress: computeAggregateProgress(
        tracker.totalSegments,
        tracker.completedSegments,
        0,
      ),
      message: `Segment ${tracker.completedSegments}/${tracker.totalSegments} complete`,
    });

    // If more segments remain, chain the next one
    if (tracker.completedSegments < tracker.totalSegments) {
      const nextIndex = tracker.completedSegments;
      try {
        // Extract last frame from the completed segment
        const lastFrameBase64 = await this.extractLastFrame(resultFilePath);

        // Create the next segment as img2video using the last frame
        const nextPayload: MediaJobPayload = {
          ...
          parentJobId,
          audio: false, // Audio only on final stitched video
        };

        const nextSegmentJob = this.repo.createJob({
          type: "img2video",
          payload: nextPayload,
          model: tracker.parentPayload.model_repo ?? undefined,
          priority: 10,
        });

        tracker.segmentJobIds.push(nextSegmentJob.id);
        logger.info(
          `[QueueMaster] Chaining segment ${nextIndex + 1}/${tracker.totalSegments}: ${nextSegmentJob.id}`,
        );
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        logger.error(
          `[QueueMaster] Failed to chain segment ${nextIndex + 1} for parent ${parentJobId}: ${msg}`,
        );
        // Retry once
        try {
          const lastFrameBase64 = await this.extractLastFrame(resultFilePath);
          const retryPayload: MediaJobPayload = {
            ...tracker.parentPayload,
            init_image: lastFrameBase64,
            image_strength: 0.8,
            num_frames: MAX_VIDEO_FRAMES,
          ...
          const retryJob = this.repo.createJob({
            type: "img2video",
            payload: retryPayload,
            model: tracker.parentPayload.model_repo ?? undefined,
            priority: 10,
          });
          tracker.segmentJobIds.push(retryJob.id);
          logger.info(
            `[QueueMaster] Retry chain segment ${nextIndex + 1}: ${retryJob.id}`,
          );
        } catch (retryErr) {
          const retryMsg =
            retryErr instanceof Error ? retryErr.message : String(retryErr);
          logger.error(
            `[QueueMaster] Retry also failed for parent ${parentJobId}: ${retryMsg} — failing parent job`,
          );
          this.repo.markFailed(
            parentJobId,
            `Segment chaining failed at segment ${nextIndex + 1}: ${retryMsg}`,
          );
          const parentJob = this.repo.getJob(parentJobId);
          if (parentJob) this.emit("job:failed", parentJob, retryMsg);
          this.segmentTracker.delete(parentJobId);
          return;
        }
      }
    } else {
      // All segments complete — stitch them together
      await this.stitchSegments(parentJobId, tracker);
    }
  }

  /**
   * Extract the last frame of a video file as a base64 PNG.
   * Uses the /last-frame endpoint on the worker sidecar.
   */
  private async extractLastFrame(videoPath: string): Promise<string> {
    ...
    if (!res.ok) {
      const text = await res.text().catch(() => "");
      throw new Error(`/last-frame returned ${res.status}: ${text}`);
    }
  ...
  /**
   * Stitch all segment videos into a single final video using ffmpeg concat
   * with 0.5s crossfade transitions. Runs on the Node.js server.
   */
  private async stitchSegments(
    ...
    const validFiles = tracker.segmentFiles.filter(
      (f): f is string => f !== null,
    );
    if (validFiles.length < 2) {
      logger.warn(
        `[QueueMaster] Parent ${parentJobId}: only ${validFiles.length} files to stitch — treating as single segment`,
      );
      if (validFiles.length === 1) {
        // Just mark the parent complete with the single segment's file
        const parentJob = this.repo.getJob(parentJobId);
        if (parentJob) {
          const filename = path.basename(validFiles[0]);
          this.repo.markComplete(
            parentJobId,
            `/api/queue/assets/file/${filename}`,
            { segments: 1 },
          );
          this.emit("job:complete", { ...parentJob, status: "complete" });
        }
      }
      this.segmentTracker.delete(parentJobId);
      return;
    ...
    this.reportProgress(parentJobId, {
      stage: "stitching",
      progress: 95,
      message: `Stitching ${validFiles.length} segments with crossfade...`,
    });

    const galleryDir =
      this.config.galleryDir ?? path.join(os.homedir(), ".openzigs", "gallery");
    await fs.mkdir(galleryDir, { recursive: true });
    const safeId = parentJobId.replace(/[^a-zA-Z0-9_-]/g, "_");
    const outputPath = path.join(galleryDir, `${safeId}_stitched.mp4`);

    try {
      await this.ffmpegConcatWithCrossfade(validFiles, outputPath, 0.5);
      logger.info(
        `[QueueMaster] Stitched ${validFiles.length} segments into ${outputPath}`,
      );

      // If audio was requested, submit audio post-processing
      if (tracker.parentPayload.audio) {
        await this.runAudioPostProcessing(
          parentJobId,
          outputPath,
          tracker.parentPayload,
        );
        return; // Audio processing will complete the parent job
      }
      ...
      if (parentJob) {
        const metadata = {
          segments: validFiles.length,
          duration: validFiles.length * SEGMENT_DURATION_SEC,
        };
        const filename = path.basename(outputPath);
        const galleryAssetId = this.repo.createAsset({
          type: "video",
          filename,
          filePath: outputPath,
          mimeType: "video/mp4",
          fileSizeBytes: (await fs.stat(outputPath)).size,
          durationSeconds: validFiles.length * SEGMENT_DURATION_SEC,
          prompt: tracker.parentPayload.prompt,
          model: parentJob.requiredModel,
          generationParams: metadata,
          source: "generated",
          jobId: parentJobId,
          projectId: parentJob.projectId ?? undefined,
        });
        this.repo.markComplete(
          parentJobId,
          `/api/queue/assets/file/${filename}`,
        ...
        this.emit("job:complete", { ...parentJob, status: "complete" });
        logger.info(
          `[QueueMaster] Multi-segment parent ${parentJobId} complete (${validFiles.length} segments, asset ${galleryAssetId})`,
        );
      }
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      logger.error(
        `[QueueMaster] Stitching failed for parent ${parentJobId}: ${msg}`,
      );
      this.repo.markFailed(parentJobId, `Segment stitching failed: ${msg}`);
      const parentJob = this.repo.getJob(parentJobId);
      if (parentJob) this.emit("job:failed", parentJob, msg);
    } finally {
  ...
  /**
   * Concatenate video files with crossfade transitions using ffmpeg.
   * Uses the xfade filter for smooth transitions between segments.
   */
  async ffmpegConcatWithCrossfade(
    inputFiles: string[],
    outputPath: string,
    ...
    // Build ffmpeg xfade filter chain for N inputs
    const inputs: string[] = [];
    for (const f of inputFiles) {
      inputs.push("-i", f);
    }

    // Each segment is ~4s. Crossfade overlaps clips.
    const segDuration = SEGMENT_DURATION_SEC;
    let filterComplex = "";
    let lastOutput = "[0:v]";

    for (let i = 1; i < inputFiles.length; i++) {
      const offset = segDuration * i - crossfadeDuration * i;
      const output = i < inputFiles.length - 1 ? `[v${i}]` : "[vout]";
      filterComplex += `${lastOutput}[${i}:v]xfade=transition=fade:duration=${crossfadeDuration}:offset=${Math.max(0, offset)}${output};`;
      lastOutput = output;
    }
  ...
  /**
   * After stitching, run audio generation on the final video.
   * Dispatches a TTS job to the video worker with the stitched video as context.
   */
  private async runAudioPostProcessing(
    parentJobId: string,
    videoPath: string,
    parentPayload: MediaJobPayload,
  ): Promise<void> {
    this.reportProgress(parentJobId, {
      stage: "audio",
      progress: 97,
      message: "Generating synchronized audio for stitched video...",
    });

    try {
      // Re-submit the stitched video back to the worker for audio-only generation
      const nodeConfig = await this.getLiveNodeConfig("m2-pro");
      const headers: Record<string, string> = {
        "Content-Type": "application/json",
      };
      ...
      const body: Record<string, unknown> = {
        job_id: `${parentJobId}_audio`,
        type: "txt2video",
        prompt: parentPayload.prompt,
        width: parentPayload.width ?? 768,
        height: parentPayload.height ?? 512,
        num_frames: MAX_VIDEO_FRAMES,
        fps: parentPayload.fps ?? 24,
        model: "ltx-2",
        callback_url: this.config.callbackUrl,
        pipeline: parentPayload.pipeline ?? "distilled",
        audio: true,
        tiling: parentPayload.tiling ?? "aggressive",
        init_image: videoBase64,
        image_strength: 1.0,
      };

      if (parentPayload.model_repo) body.model_repo = parentPayload.model_repo;
      ...
      if (res.status !== 202 && !res.ok) {
        throw new Error(`Audio post-processing dispatch failed: ${res.status}`);
      }

      logger.info(
        `[QueueMaster] Audio post-processing dispatched for parent ${parentJobId}`,
      );
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      logger.warn(
        `[QueueMaster] Audio post-processing failed for ${parentJobId}, completing without audio: ${msg}`,
      );
      // Still mark parent as complete without audio
      const parentJob = this.repo.getJob(parentJobId);
      if (parentJob) {
        const filename = path.basename(videoPath);
        const metadata = {
          segments: this.segmentTracker.get(parentJobId)?.totalSegments ?? 1,
          audio_error: msg,
        };
        this.repo.markComplete(
          parentJobId,
  ...
  /**
   * Report progress for multi-segment jobs, including segment indicator.
   */
  reportSegmentProgress(
    ...
    this.reportProgress(parentJobId, {
      stage: "generating",
      progress: aggregate,
      message: `Segment ${segmentIndex + 1}/${totalSegments} — ${segmentProgress}%`,
    });
  }
  ...
  async handleJobCompletion(
    ...
    // Multi-segment: if this is a segment sub-job, route to segment chaining
    if (job.payload.parentJobId && job.payload.segmentIndex !== undefined) {
      const filePath = (result.metadata?.file_path as string) ?? resultUrl;
      // Derive actual file path from resultUrl or metadata
      const galleryDir =
        this.config.galleryDir ??
        path.join(os.homedir(), ".openzigs", "gallery");
      const segmentFilePath =
        typeof filePath === "string" && filePath.startsWith("/")
          ? filePath
          : path.join(galleryDir, path.basename(resultUrl));
      await this.handleSegmentCompletion(jobId, job, segmentFilePath);
