...
describe("YouTubePublishService", () => {
  ...
  describe("publish", () => {
    ...
    it("returns failed status when yt_upload_video tool is not available", async () => {
      ...
      // Create a temp file to simulate an existing render
      const tmpFile = "/tmp/test-yt-publish-video.mp4";
      const fsMod = await import("node:fs");
      fsMod.writeFileSync(tmpFile, "fake video data");
    ...
    it("publishes successfully when tool succeeds", async () => {
      ...
      const tmpFile = "/tmp/test-yt-publish-success.mp4";
      const fsMod = await import("node:fs");
      fsMod.writeFileSync(tmpFile, "fake video data");
    ...
    it("handles tool error response", async () => {
      ...
      const tmpFile = "/tmp/test-yt-publish-error.mp4";
      const fsMod = await import("node:fs");
      fsMod.writeFileSync(tmpFile, "fake video data");
    ...
    it("handles API error in response JSON", async () => {
      ...
      const tmpFile = "/tmp/test-yt-publish-api-error.mp4";
      const fsMod = await import("node:fs");
      fsMod.writeFileSync(tmpFile, "fake video data");
    ...
    it("emits Socket.IO events during publish", async () => {
      ...
      const tmpFile = "/tmp/test-yt-publish-events.mp4";
      const fsMod = await import("node:fs");
      fsMod.writeFileSync(tmpFile, "fake video data");
