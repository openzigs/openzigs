...
describe("twitter-tools", () => {
  ...
  describe("when twitter server is running", () => {
    it("delegates correctly for twitter-get-me", async () => {
      ...
      expect(mgr.callTool).toHaveBeenCalledWith("twitter", "twitter_get_me", expect.any(Object));
    ...
    it("delegates correctly for twitter-post-tweet", async () => {
      ...
      expect(mgr.callTool).toHaveBeenCalledWith("twitter", "twitter_post_tweet", { text: "Hello world" });
      expect(result.text).toBe('{"id":"12345"}');
    });

    it("delegates correctly for twitter-search-tweets", async () => {
      const mgr = createMockLocalServerManager({ callResult: { text: '{"data":[]}' } });
      const tools = createTwitterTools({ localServerManager: mgr as never });
      const handler = tools.find((t) => t.name === "twitter-search-tweets")!.handler;
      const result = await handler({ query: "AI agents", max_results: 20 });
      expect(mgr.callTool).toHaveBeenCalledWith("twitter", "twitter_search_tweets", { query: "AI agents", max_results: 20 });
      expect(result.text).toBe('{"data":[]}');
    });

    it("delegates correctly for twitter-get-tweet", async () => {
      const mgr = createMockLocalServerManager({ callResult: { text: '{"data":{"id":"999"}}' } });
      const tools = createTwitterTools({ localServerManager: mgr as never });
      const handler = tools.find((t) => t.name === "twitter-get-tweet")!.handler;
      const result = await handler({ tweet_id: "999" });
      expect(mgr.callTool).toHaveBeenCalledWith("twitter", "twitter_get_tweet", { tweet_id: "999" });
    ...
    it("delegates correctly for twitter-get-user-tweets", async () => {
      const mgr = createMockLocalServerManager({ callResult: { text: '{"data":[]}' } });
      const tools = createTwitterTools({ localServerManager: mgr as never });
      const handler = tools.find((t) => t.name === "twitter-get-user-tweets")!.handler;
      const result = await handler({ user_id: "123", max_results: 5 });
      expect(mgr.callTool).toHaveBeenCalledWith("twitter", "twitter_get_user_tweets", { user_id: "123", max_results: 5 });
      expect(result.text).toBe('{"data":[]}');
    });
    ...
    it("delegates correctly for twitter-get-dm-events", async () => {
      const mgr = createMockLocalServerManager({ callResult: { text: '{"data":[]}' } });
      const tools = createTwitterTools({ localServerManager: mgr as never });
      const handler = tools.find((t) => t.name === "twitter-get-dm-events")!.handler;
      const result = await handler({ max_results: 10 });
      expect(mgr.callTool).toHaveBeenCalledWith("twitter", "twitter_get_dm_events", { max_results: 10 });
      expect(result.text).toBe('{"data":[]}');
    });

    it("delegates correctly for twitter-send-dm", async () => {
      const mgr = createMockLocalServerManager({ callResult: { text: '{"sent":true}' } });
      const tools = createTwitterTools({ localServerManager: mgr as never });
      const handler = tools.find((t) => t.name === "twitter-send-dm")!.handler;
      const result = await handler({ participant_id: "456", text: "Hello!" });
      expect(mgr.callTool).toHaveBeenCalledWith("twitter", "twitter_send_dm", { participant_id: "456", text: "Hello!" });
      expect(result.text).toBe('{"sent":true}');
    });

    it("delegates correctly for twitter-get-user", async () => {
      const mgr = createMockLocalServerManager({ callResult: { text: '{"data":{"username":"testuser"}}' } });
      const tools = createTwitterTools({ localServerManager: mgr as never });
      const handler = tools.find((t) => t.name === "twitter-get-user")!.handler;
      const result = await handler({ username: "testuser" });
      expect(mgr.callTool).toHaveBeenCalledWith("twitter", "twitter_get_user", { username: "testuser" });
      expect(result.text).toBe('{"data":{"username":"testuser"}}');
    });

    it("returns error from server", async () => {
      const mgr = createMockLocalServerManager({ callResult: { text: "API rate limit exceeded", isError: true } });
      const tools = createTwitterTools({ localServerManager: mgr as never });
      const handler = tools.find((t) => t.name === "twitter-get-me")!.handler;
      const result = await handler({});
      expect(result.isError).toBe(true);
  ...
  describe("risk levels", () => {
    it("read operations are low risk", () => {
      const tools = createTwitterTools({});
      const lowRiskTools = ["twitter-get-me", "twitter-get-user-tweets", "twitter-search-tweets", "twitter-get-tweet", "twitter-get-user"];
    ...
    it("write operations are high risk", () => {
      const tools = createTwitterTools({});
      const highRiskTools = ["twitter-post-tweet", "twitter-get-dm-events", "twitter-send-dm"];
