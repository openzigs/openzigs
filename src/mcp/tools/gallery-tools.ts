...
const queryGalleryAssetsSchema = z.object({
  ...
  source: z
    .enum(["generated", "uploaded", "ingested"])
    .optional()
    .describe("Filter by asset source"),
  project_id: z.string().optional().describe("Filter by project ID"),
  search: z
    .string()
    .optional()
    .describe("Full-text search across prompt, filename, and tags"),
  model: z
    .string()
    .optional()
    .describe("Filter by generation model (e.g., 'flux-schnell', 'ltx-2')"),
  asset_id: z
    .string()
    .optional()
    .describe("Get a single asset by ID instead of listing"),
  limit: z
    .number()
    .min(1)
    .max(100)
...
export const createGalleryTools = ({
  mediaQueueRepo,
}: GalleryToolsOptions): ToolDefinition[] => {
  return [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          search: {
            type: "string",
            description: "Full-text search across prompt, filename, tags",
          },
          model: { type: "string" },
      ...
      handler: async (args) => {
        try {
          ...
          if (input.asset_id) {
            ...
            if (!asset)
              return {
                text: `Asset '${input.asset_id}' not found.`,
                isError: true,
              };
          ...
          if (input.model) {
            results = results.filter(
              (a: Record<string, unknown>) =>
                ...
                a.model.toLowerCase().includes(input.model!.toLowerCase()),
            );
          }
          ...
          if (input.search) {
            ...
            results = results.filter((a: Record<string, unknown>) => {
              const haystack = [a.prompt, a.filename, a.tags]
                .filter(Boolean)
                .join(" ")
                .toLowerCase();
          ...
          return {
            text: JSON.stringify(
              { count: results.length, assets: results },
              null,
              2,
            ),
          };
          ...
          return {
            text: `Error querying gallery: ${err instanceof Error ? err.message : String(err)}`,
            isError: true,
          };
        }
      },
