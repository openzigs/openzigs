...
export class WebChatChannel implements MessageChannel {
  ...
  private handleConnection(socket: Socket) {
    ...
    socket.on(
      ...
      (data: {
        ...
        const wasFreeform =
          typeof data?.wasFreeform === "boolean" ? data.wasFreeform : true;
        if (!requestId) return;
    ...
    socket.on("chat:clear", () => {
      if (this.sessionManager) {
        void this.clearSessionHistory(userId)
          .then((cleared) => {
    ...
    socket.on("chat:restore-session", (data: { sessionId?: string }) => {
      ...
      void this.restoreSession(socket, sessionId).catch(() => {
        socket.emit("chat:error", {
          error: `Failed to restore session ${sessionId}`,
        });
      });
    });
  ...
  private async sendSessionHistory(
    socket: Socket,
    userId: string,
  ): Promise<void> {
