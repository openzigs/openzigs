...
echo ""
echo "Music generation (ACE-Step 1.5 — Apple Silicon):"
echo "  - Requires Python 3.11.x (brew install python@3.11) and uv."
echo "  - Also requires a local clone of the ACE-Step Apple Silicon fork:"
echo "      git clone --depth 1 https://github.com/clockworksquirrel/ace-step-apple-silicon.git ~/ace-step-apple-silicon"
echo "      cd ~/ace-step-apple-silicon && uv sync"
echo "  - Sidecar setup (for the HTTP API wrapper):"
echo "      cd sidecars/music"
echo "      /opt/homebrew/bin/python3.11 -m venv .venv && source .venv/bin/activate"
echo "      pip install -r requirements.txt"
echo "  - Start manually: cd sidecars/music && source .venv/bin/activate && python server.py"
echo "  - Configure in Admin → Music Generation Node (default port 5009)"
