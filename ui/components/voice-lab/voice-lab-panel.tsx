...
function F5TTSClipCard({
  ...
  const handlePlayRef = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/audio/f5tts/clips/${clip.id}/audio`);
      if (!res.ok) {
...
function F5TTSAddClipForm({
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      const res = await fetch("/api/admin/audio/upload/f5tts-ref-audio", {
        method: "POST",
        headers: {
...
function F5TTSProfileCard({
  ...
  const handleTest = async () => {
    ...
    setTesting(true);
    try {
      const res = await fetch(`/api/admin/audio/f5tts/profiles/${profile.id}/test`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
export function VoiceLabPanel() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Full-width: F5-TTS Profiles (Engine C) ── */}
      <div className="rounded-xl border border-border bg-card p-4">
        <F5TTSSection />
      </div>
    </div>
