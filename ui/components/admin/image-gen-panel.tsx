...
export const ImageGenPanel = () => {
  const queryClient = useQueryClient();

  const configQuery = useQuery({
    queryKey: ["image-gen-config"],
    queryFn: () => fetchJson<ImageGenConfig>("/api/admin/image-gen/config"),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (body: Record<string, string>) =>
      fetchJson("/api/admin/image-gen/config", {
        method: "PUT",
  ...
  const handleHealthCheck = async () => {
    ...
    try {
      ...
      const result = await fetchJson<HealthResult>(
        `/api/admin/image-gen/health${qs ? `?${qs}` : ""}`,
      );
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          {mode === "local"
            ? "Image generation runs on this machine (localhost:5005)."
            : "Image generation is offloaded to a remote FluxQ node on your network."}
        </p>
      </div>

      {/* Network Node Config (shown only in network mode) */}
      {mode === "network" && (
