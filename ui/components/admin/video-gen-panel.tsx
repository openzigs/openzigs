...
export const VideoGenPanel = () => {
  const queryClient = useQueryClient();

  const configQuery = useQuery({
    queryKey: ["video-gen-config"],
    queryFn: () => fetchJson<VideoGenConfig>("/api/admin/video-gen/config"),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (body: Record<string, string>) =>
      fetchJson("/api/admin/video-gen/config", {
        method: "PUT",
        body: JSON.stringify(body),
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["video-gen-config"] });
      showToast("Video generation settings saved", "success");
    },
  ...
  const handleHealthCheck = async () => {
    ...
    try {
      ...
      const result = await fetchJson<HealthResult>(
        `/api/admin/video-gen/health${qs ? `?${qs}` : ""}`,
      );
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          {mode === "local"
            ? "Video generation runs on this machine (localhost:5007)."
            : "Video generation is offloaded to a remote LTX-2 worker on your network."}
        </p>
      </div>
      ...
      {mode === "network" && (
        <div className="space-y-3 rounded-lg border border-border bg-muted/30 p-4">
          <div className="space-y-1.5">
            ...
            <input
              type="text"
              placeholder="http://192.168.1.50:5007"
              value={nodeUrl}
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {healthResult && (
          <span className={`flex items-center gap-1.5 text-sm font-medium ${healthResult.ok ? "text-green-500" : "text-red-500"}`}>
            ...
            {healthResult.ok
              ? `Connected${healthResult.loaded_model ? ` · ${healthResult.loaded_model}` : ""}${healthResult.worker ? ` · ${healthResult.worker}` : ""}`
