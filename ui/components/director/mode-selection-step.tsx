    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-sm text-muted-foreground max-w-lg mx-auto">
          Select how you want to produce your video. Each mode uses a
          single-shot LLM call for cost-efficient, AI-driven editing decisions.
        </p>
      </div>
            ...
            >
              ...
              {isSelected && (
                <div className="absolute top-4 right-4">
                  <div
                    className={`h-3 w-3 rounded-full bg-current ${mode.accent}`}
                  />
                </div>
              )}
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                >
                  <Icon
                    className={`h-5 w-5 ${isSelected ? mode.accent : "text-muted-foreground"}`}
                  />
                </div>
                <div>
                  ...
                  <span className={`text-xs font-medium ${mode.accent}`}>
                    {mode.subtitle}
                  </span>
                </div>
              </div>
              ...
              <ul className="space-y-1.5">
                {mode.features.map((feature) => (
                  ...
                  >
                    <span
                      className={`h-1 w-1 rounded-full ${isSelected ? `bg-current ${mode.accent}` : "bg-muted-foreground/50"}`}
                    />
                    {feature}
                  </li>
                ))}
